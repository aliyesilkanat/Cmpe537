...
def myHarrisDetector(im, sigma=0.5):
    # derivatives
...
def getharrispoints(harrisim, threshold=0.04, min_dist=10):
    ...
    for i in index:
        if allowedlocations[coords[i, 0], coords[i, 1]] == 1:
            ...
            allowedlocations[(coords[i, 0] - min_dist):(coords[i, 0] + min_dist),
...
def plot_repeatability(file_names=None, sift_repeatability=None, harris_repeatability=None, manual_repeatability=None,
                       sift_results=None, harris_results=None, manual_harris_results=None,path=None):
